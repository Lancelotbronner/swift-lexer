...
extension Graph {
	...
	mutating func branch(from node: Node, _ id: NodeId) -> Node.BranchContent {
		...
		case .Seq(var content):
			return content.toBranch(graph: &self)
		}
	}
...
extension Graph {
	...
	mutating func mergeSeq(_ seq: inout Node.SeqContent, _ other: Node, _ otherId: NodeId) -> Node.SeqContent? {
		switch other {
		case .Branch(let branchContent):
			if seq.miss == nil {
				...
				if var newSeq = seq.split(at: loopCount, graph: &self) {
					newSeq.miss(anytime: otherId)
		...
		case .Seq(let otherSeq):
			if let (prefix, miss) = seq.prefix(with: otherSeq) {
				var newSeq = seq
				let newSeqId = newSeq.asRemainder(at: prefix.count, graph: &self)
				var newOther = otherSeq
				let newOtherId = newOther.asRemainder(at: prefix.count, graph: &self)

				return try! .init(seq: prefix, then: merge(newSeqId, newOtherId), miss: miss)
			}
		case .Leaf:
			if seq.miss == nil {
	...
	mutating func mergeKnown(_ a: NodeId, _ b: NodeId, _ into: NodeId) throws -> NodeId {
		...
		case (.Seq(var seq), _):
			newSeq = mergeSeq(&seq, rhs, b)
		case (_, .Seq(var seq)):
			newSeq = mergeSeq(&seq, lhs, a)
		case _:
			break
		}
		...
		flatten: while let miss = lhsContent.miss {
			...
			case .Seq(var seqContent):
				toMerge = seqContent.toBranch(graph: &self)
			case _:
				break flatten
			}
